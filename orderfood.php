...
<?php
            $_SESSION["key_order"] = array();
            $stmt = $con->prepare("SELECT * FROM `items` where not deleted");
            $stmt->execute();
            $result = $stmt->get_result();
            while($rs=$result->fetch_object()){
                array_push($_SESSION["key_order"],$rs->id );
                echo '<div class="row">
                    <div class="col-md-4 col-sm-6">
                    ...
                    <div class="col-md-4 col-sm-6">
                        <input type="number" class="" name="'.$rs->id.'" value="0">
                    </div>
                </div>';
            }
        ?>
